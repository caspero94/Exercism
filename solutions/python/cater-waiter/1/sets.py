...
def compile_ingredients(dishes):
    ...
    final_list = set()
    for l_ing in dishes:
        final_list = final_list.union(list(l_ing))
    return final_list
...
def singleton_ingredients(dishes, intersection):
    ...
    # Initialize a set to store singleton ingredients
    singletons = set()

    # Iterate over ingredient counts to find singletons
    for ingredient, count in ingredient_count.items():
        # If ingredient occurs only once, add it to singletons
        if count == 1:
            singletons.add(ingredient)

    # Return the set of singleton ingredients
    return singletons